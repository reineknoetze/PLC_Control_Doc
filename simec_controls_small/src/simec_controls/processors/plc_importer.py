...
def import_plc_module_xml(
    ...
    try:
        ...
        # Disambiguate controller name if already present (use filename stem as tag)
        _base_name = controller_name
        _tag = Path(l5x_path).stem

        import re as _re

        _tag = ''.join(ch for ch in _tag if ch.isalnum() or ch in ('_', '-'))[:32] or 'file'
        _candidate = _base_name
        _suffix = 1
        while True:
            row = conn.execute("SELECT 1 FROM plc_controllers WHERE name=?", (_candidate,)).fetchone()
            if not row:
                break
            _candidate = f"{_base_name} [{_tag}]" if _suffix == 1 else f"{_base_name} [{_tag}-{_suffix}]"
            _suffix += 1
        
        controller_name = _candidate
        ...
        conn.execute(
            ...
            (
                controller_name,
                vendor,
                root.attrib.get("SchemaRevision", ""),
                None,
                controller_hash,
                import_id,
                cfg.facility_id,
